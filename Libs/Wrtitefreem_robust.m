...
function Ks_source = Wrtitefreem_rosbust(u,Yobjtivo)
syms x t y;
...
Ks_obj = ['func Yobjtivo= 0.0*x +' ,char(simplify(subs(Yobjtivo,t,y))),';'];

Filename = ['Ks_freefem_solver/','Condiciones_Robust.edp'];
%creando el archivo que contiene la informacion para freefem
fid = fopen(Filename,'wt');
fprintf(fid, '%s\n%s\n%s', ks_ini, ks_ini_xx, Ks_obj);
fclose(fid);
